...
### v3.6.5 (2026-08-16) - 教务查询统一模块：空教室 / 考试 / 成绩

**教务查询入口统一**

- 原「查空教室」重构为「教务查询」统一入口（导航「教务查询」，URL `/dashboard/query`），内部分为空教室 / 考试 / 成绩三个页签；后端统一为 `/api/query` 会话与路由，移除旧 `/api/classroom`
- 空教室查询：按学期 / 校区 / 楼号 / 场地类别 / 座位数 / 场地名称 / 周次 / 星期 / 节次筛选，结果支持导出
- 页签通栏铺在页面内容上方（横屏 / 竖屏一致），登录成功后默认展示欢迎页；支持 URL `?tab=grade` 直达指定页签

**考试查询与导入**

- 考试查询：按学期查询考试安排，支持考试名 / 考试时间 / 学院 / 课程筛选，展示考试日期、时间、地点、座号、开课学院
- 一键导入日程：可将考试导入现有课表或新建课表（标题「课程名（考试）」、自动解析起止时间与地点）

**成绩查询与个人中心展示**

- 成绩查询：默认按当前时间自动选择学期并直接展示，无需手动点击；同时显示全部学期 GPA 与 本学期 GPA 双值，全部学期绩点计为正式 GPA
- 成绩筛选：全部 / 合格 / 不合格 / 补考 / 重修及格（依据教务系统考试性质字段）
- 支持勾选「在个人中心显示」，将学期成绩快照保存到个人中心「成绩」页签，无需再次登录教务即可查看

**教务登录与会话**

- 教务系统登录改为两步弹窗（WebVPN → 教务账号），验证码点击图片刷新，验证码超时提示 toast
- 去除本地会话超时：每次查询实时检测教务系统状态，上游返回登录失效页（含统一认证安全验证页）时统一提示「教务登录过期」并弹出重新登录弹窗
- 考试 / 成绩默认按当前学期自动查询：暑假期间自动回落到刚结束的春季学期，避免默认选中尚未开学的学期导致查空

**教务登录闸门与排队**

- 登录串行化：全局 30 秒登录闸门 + 队列上限 10 人（闸门模式凭据不入队，轮到自己时前端才提交真实登录）；同一账号 30 分钟内登录尝试达 3 次即锁定 30 分钟
- 全局熔断：检测到学校安全验证墙证据（3 次/10 分钟）自动暂停登录 1-5 分钟，避免触发学校 CAS 风控误伤
- 登录排队 UX：教务登录弹窗展示「当前排位 / 预计等待」，轮到时自动提交；队列项超时自动重新入队（最多 2 次），关闭弹窗即取消排队
- 会话保活：成功登录的教务会话每 25 分钟轻量探测保活（多 worker 由 Redis 分布式锁选举单一执行者），期间检测到安全验证墙自动标记过期并上报熔断
- Cookie 加密存储：教务 cookie jar 经 Fernet 加密后写入数据库与 Redis 热缓存（`jwxt:session:*`），密钥只存服务器文件/环境变量
- Redis 部署：docker-compose 新增 `redis:7-alpine` 服务（独立 internal 网络、健康检查），应用与 Redis 同网共享闸门/队列/熔断/缓存状态

**并发与安全修复**

- 锁所有权：登录闸门与保活分布式锁携带随机 owner token，释放按 token 比对（compare-and-delete），旧持有者/过期任务无法误删新持有者的锁
- 原子化：排队（入队/查队/队首消费）、熔断计数、会话缓存投影、保活扫描认领全部改为 Redis Lua 原子脚本，进程内降级模式提供等价单锁实现
- Redis fail-closed：配置了 `REDIS_URL` 但连接失败时拒绝启动；运行期闸门故障对登录返回 503
- 密钥安全：Fernet 密钥文件原子创建（O_EXCL+0600）；加密失败直接报错；密钥文件排除出 Docker 构建上下文与镜像
- 会话串行化：同一用户的教务操作（登录/验证码/查询/保活探测）按用户互斥执行；保活扫描改为每用户短事务
- 版本保护：会话持久化与失效标记采用 `updated_at` 版本 CAS 且单事务提交，旧保活/旧探测无法覆盖刚成功的新登录
- 数据库连接释放：查询/登录/保活路由在上游教务 HTTP 期间关闭并归还数据库连接，持久化/失效标记改用短事务
- 导入会话加固：课表导入会话移入共享加密存储、绑定创建用户、原子领取；ZFW 导入改为单事务
- 认证加固：旧密码哈希升级改用版本 CAS；`token_version` 改为 SQL 侧原子自增；生产 `SECRET_KEY` 强制 ≥32 字符且拒绝占位符；邮箱唯一性下沉为大小写不敏感唯一索引；验证码邮件新增按 IP 每小时配额
- 幂等写入：分享课表重复导入返回原课表（导入台账）；ICS 导入按 `source_uid` 幂等更新；批量/智能排班支持 `Idempotency-Key` 请求头
- 团队并发：批量删除/成员管理按团队行锁 + 稳定升序处理，消除并发反向锁环死锁；排班执行在行锁内重新检查冲突；预览接口改为只读；团队转让带 creator CAS 校验
- 公开分享最小化：公开分享响应剔除学号/班级/年级/学院等个人信息；分享权限枚举化（未知值直接 422）
- 前端流程治理：登录/排队流程携带代际标记，迟到的轮询/登录响应不再写回状态；「记住密码」缓存增加 7 天 TTL 与代际保护；401 处理按请求发出时的 token 比对

**站点配置与稳定性修复**

- 站点配置新增「默认开学日期」：管理员可在系统设置 → 站点配置设置默认开学日期（保存到 `config.toml`），用户创建课表时自动预填该日期（教务 / ICS 导入新建课表同样生效），未配置时回退到创建当天
- 修复：教务登录排队人数达到上限（10 人）时正确返回 429 与「稍后重试」提示，此前该场景会因路由内变量遮蔽 `fastapi.status` 返回 500；新增路由层回归测试

**数据迁移与部署**

- 新增 5 个 Alembic revision（现役 head `d3e5f7a9b1c3`）：邮箱唯一索引、`email_send_logs`、`schedule_share_imports`、`events.source_uid`、批量/排班 `idempotency_key`
- PostgreSQL 部署需执行 `alembic upgrade head` 后再启动；生产直连教务端点拒绝明文 HTTP（除非显式 `JWXT_ALLOW_PLAIN_HTTP=1`）
